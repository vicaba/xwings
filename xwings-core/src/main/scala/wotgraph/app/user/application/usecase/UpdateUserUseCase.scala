...
class UpdateUserUseCase(userRepository: UserRepository, hash: Hasher.PreconfiguredHash, authorizationService: AuthorizationService) {

  def execute(id: String, u: CreateUser)(executorAgentId: UUID): Future[User Or Every[AppError]] = {
    AuthorizationService.asyncExecute(authorizationService, executorAgentId, UpdateUserUseCase.permission.id) {

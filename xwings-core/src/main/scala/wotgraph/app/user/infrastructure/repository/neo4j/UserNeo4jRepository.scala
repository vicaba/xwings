...
case class UserNeo4jRepository(
  ...
  def findById(id: UUID): Future[Option[User]] = {
    ...
    val query =
      s"""
         |${Keywords.Match} (n:$UserLabel { $Id: ${n(Id)} })-[r:$RoleRelKey]->(n2)
         |RETURN n.$Id AS $Id, n.$Name AS $Name,
         |n2.${RoleKeys.Id} AS $roleIdKey n2.${RoleKeys.Name} AS $roleNameKey
         |""".stripMargin
  ...
  def findByCredentials(name: String, password: String): Future[Option[User]] = {
    ...
    val query =
      s"""
         |${Keywords.Match} (n:$UserLabel { $Name: ${n(Name)}, $Password: ${n(Password)} })-[r:$RoleRelKey]->(n2)
         |RETURN n.$Id AS $Id, n.$Name AS $Name,
         |n2.${RoleKeys.Id} AS $roleIdKey, n2.${RoleKeys.Name} AS $roleNameKey
         |""".stripMargin
  ...
  def create(user: User): Future[User Or Every[StorageError]] = {
    ...
    def createQuery: LazyResult = {
      ...
      val query =
        s"""
           |${Keywords.Match} (role:${RoleNeo4jRepository.Keys.RoleLabel}) WHERE role.${RoleKeys.Id} = ${n(roleIdKey)}
           |${Keywords.Create} (n:$UserLabel { $Id: ${n(Id)}, $Name: ${n(Name)}, $Password: ${n(Password)} })-[r:$RoleRelKey]->(role)
           |""".stripMargin
  ...
  def update(user: User): Future[User Or Every[StorageError]] = {
    ...
    def createRoleRelationAndUpdateUserQuery: LazyResult = {
      ...
      val query =
        s"""
           |${Keywords.Match} $currentUserMatch, $roleMatch
           |${Keywords.Create} $relationshipCreate SET n.$Name = ${n(Name)}, n.$Password = ${n(Password)}
           |""".stripMargin
